...
if [[ -z "${IAST_MANAGER_URL}" ]]; then
  java -jar java-rabbitmq-entry-point.jar
else
  cwd=$(pwd)
  ...
  cd ${cwd}
  java -javaagent:/usr/local/cxiast-agent/cx-launcher.jar -Xverify:none -Dcx.standalone=yes -jar java-rabbitmq-entry-point.jar
fi
