...
if [[ -z "${IAST_MANAGER_URL}" ]]; then
  java -jar java-rabbitmq-propagator.jar
else
  cwd=$(pwd)
  ...
  cd ${cwd}
  java -javaagent:/usr/local/cxiast-agent/cx-launcher.jar -Xverify:none -Dcx.standalone=yes -jar java-rabbitmq-propagator.jar
fi
